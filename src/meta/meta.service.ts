...
export class MetaService {
  ...
  getAuthorizationUrl(redirectUri: string, state: string): string {
    ...
    return `https://www.facebook.com/${this.META_VERSION_API}/dialog/oauth?client_id=${appId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${state}&scope=${scopes}`;
  }
  ...
  async exchangeCodeForToken(code: string, redirectUri: string): Promise<any> {
    ...
    const response = await firstValueFrom(
      this.httpService.get(
        `https://graph.facebook.com/${this.META_VERSION_API}/oauth/access_token`,
        {
          params: {
            client_id: appId,
  ...
  async getPageAccessToken(
    ...
    const response = await firstValueFrom(
      this.httpService.get(
        `https://graph.facebook.com/${this.META_VERSION_API}/${pageId}`,
        {
          params: {
            fields: 'access_token',
  ...
  async getUserPages(accessToken: string): Promise<any[]> {
    const response = await firstValueFrom(
      this.httpService.get(
        `https://graph.facebook.com/${this.META_VERSION_API}/me/accounts`,
        {
          params: {
